...
function Modal() {
    ...
    const [showModal,
        setShowModal] = useRecoilState(modalState)
    ...
    useEffect(() => {
        ...
        async function fetchMovie() {
            const data = await fetch(`https://api.themoviedb.org/3/${movie?.media_type === 'tv' ? 'tv' : 'movie'
                ...
                }&language=en-US&append_to_response=videos`).then((response) => response.json()).catch((err) => console.log(err.message))
            //setData(data);
            // console.log(data);
            if (data?.videos) {
        ...
        <div className='flex space-x-16 rounded-b-md bg-[#181818]'>
            <div className='space-y-6 text-lg'>
                <div className='flex items-center space-x-2 text-sm'>
                    <p className='font-semibold text-green-400'>{movie!.vote_average * 10}% March</p>
